...
test("profile resolution retries reuse one generated request ID and then poll existing work", async () => {
  ...
  const responses = [
    ...
    // Search omits the profile, so resolution falls back to the idempotent profile read.
    jsonResponse({ search_id: "search-1", status: "completed", results: [{ profile_id: "profile-1", status: "ready", generation_level: 3 }] }),
...
test("profile-read retries retain one idempotency header and independent reads use new keys", async () => {
  ...
  const responses = [jsonResponse({ error: "temporary failure" }, 503), jsonResponse(profile), jsonResponse(profile)];
  const client = new ProfileOrbitClient({
    apiKey: "sk_orb_test",
    fetchImpl: async (url, init) => {
      calls.push({ url: String(url), init });
